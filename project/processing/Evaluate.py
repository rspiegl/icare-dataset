...
def draw_plots(trim: np.ndarray, cali_trim: np.ndarray, image_path: str, save_path: str, trim_fix: dict,
    ...
    if trim is not None:
        ...
        axes[0][1].hist2d(trim[0], trim[1], bins=BINS, range=RANGE_PIC, alpha=0.6, zorder=2, cmin=0.01)
    ...
    if cali_trim is not None:
        ...
        axes[0][2].hist2d(cali_trim[0], cali_trim[1], bins=BINS, range=RANGE_PIC, alpha=0.6, zorder=2, cmin=0.01)
...
def draw_plots_2(trim_fix: dict, cali_trim_fix: dict, stat_string:str, image_path: str, save_path: str,):
    img = Image.open(image_path)
    img = img.resize(PLOT_SIZE)
...
def create_plots_2(ids=(3, 15), dats=DATASETS):
        ...
        for d in dats:
            ...
            for im, cali, im_path in ici:
                ...
                if len(trim_fix['x']) > 0 and len(cali_trim_fix['x']) > 0:
                    draw_plots_2(trim_fix, cali_trim_fix, stat_string, im_path, good_path)
...
def fix_trial_number(ids=(3, 15), dats=DATASETS):
    ...
    for i in range(ids[0], ids[1]):
        for d in dats:
            ...
            for im, cali, im_path in ici:
                image_name = re.findall(IMAGE_NAME_REGEX, im_path)[0]
                trial_number = re.findall("(\d{1,2})_", im_path)[0]
